...
class BlockDevice(object):
    ...
    def run_cmd(self, cmd_spec, expected_rc=0, timeout=7200, tries=3):
        return util.run_cmd(cmd_spec, expected_rc=0, timeout=7200, tries=3)
    ...
    def _is_remote_path_exist(self, path):
    ...
    def _validate_device(self, device):
        if not self._is_remote_path_exist(device):
            self._mknod_device(device)

    def _validate_directory(self, directory):
        if not self._is_remote_path_exist(directory):
...
class RemoteBlockDevice(SSHManager, BlockDevice):
    """block device related ops"""

    def __init__(self, ip, user='root', pwd='password', key_file=None):
        super(RemoteBlockDevice, self).__init__(self, ip, user, pwd, key_file)

    def run_cmd(self, cmd_spec, expected_rc=0, timeout=7200, tries=3):
        return self.ssh_cmd(cmd_spec, expected_rc=0, timeout=7200, tries=3)


class RemoteBlockDevice1(SSHManager):
    """block device related ops"""

    def __init__(self, ip, user='root', pwd='password', key_file=None):
        SSHManager.__init__(self, ip, user, pwd, key_file)
    ...
    def get_all_devices(self, pattern='/dev/*'):
        rc, output = self.ssh_cmd('ls %s' % pattern, expected_rc='ignore')
        logger.info(output)
        device_list = output.strip('\n').split(' ')
        return device_list
    ...
    def _is_remote_path_exist(self, path):
        ls_cmd = 'ls {0}'.format(path)
        rc, output = self.ssh_cmd(ls_cmd, expected_rc='ignore')
        if 'No such file or directory' in output:
            logger.warning(output)
            return False
        else:
    ...
    def _validate_directory(self, directory):
        if not self._is_remote_path_exist(directory):
            logger.info('Create a new one')
            mkdir_cmd = 'mkdir -p {0}'.format(directory)
            self.ssh_cmd(mkdir_cmd, expected_rc=0)
    ...
    def mount_fs(self, source, target, types, options='discard'):
        ...
        self._validate_directory(target)
        cmd = 'mount -t %s -o %s %s %s ' % (types, options, source, target)
        rc, output = self.ssh_cmd(cmd, expected_rc=0, tries=3)
        logger.info(output)
        return rc
